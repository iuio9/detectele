...
class InferenceWorker(QObject):
    ...
    def run_video_inference(self):
        ...
        while cap.isOpened() and not self.should_stop:
            ...
            results = self.model(frame, imgsz=self.img_size, conf=self.conf_thresh, iou=self.iou_thresh)
        ...
        out.release()
        self.image_update.emit(output_path)
        self.log_update.emit(f"视频处理完成，保存路径: {output_path}")
        self.stats_update.emit(f"处理帧数: {processed} 帧")
        self.progress_update.emit(100)

    def run_camera_inference(self):
        """Run inference from live camera."""
        self.log_update.emit("从摄像头实时推理中...")
        ...
        while cap.isOpened() and not self.should_stop:
            ...
            if frame_id % 10 == 0:
                self.image_update.emit(output_path)
                self.progress_update.emit(min(100, frame_id))  # 模拟进度条

        cap.release()
        out.release()
        self.image_update.emit(output_path)
        self.log_update.emit(f"摄像头实时推理完成，保存路径: {output_path}")
        self.stats_update.emit(f"总帧数: {frame_id}")
        self.progress_update.emit(100)
