...
class InferenceTab(QWidget):
    ...
    def setup_ui(self):
        ...
        self.camera_radio = QRadioButton("摄像头模式")
        ...
        # 设置默认图像（首次显示）
        default_image_path = os.path.join(os.path.dirname(__file__), '../assets/moren.jpg')
        if os.path.exists(default_image_path):
            ...
            self.image_display.setText("等待检测...")
    ...
    def play_result_video(self, video_path):
        ...
        if not os.path.exists(video_path):
            self.log_message(f"视频文件不存在: {video_path}")
            return
        ...
        self.status_label.setText("状态: 正在播放视频")
        self.status_label.setStyleSheet("font-weight: bold; color: #009688;")
        self.log_message(f"播放推理视频: {video_path}")
    ...
    def update_mode(self, checked):
        if self.image_radio.isChecked():
            ...
            self.input_btn.setText("选择视频文件...")
            self.input_edit.setPlaceholderText("选择要检测的视频")
        elif self.camera_radio.isChecked():
            self.input_btn.setText("打开摄像头")
            self.input_edit.setPlaceholderText("摄像头编号，如 0")
    ...
    def validate_inputs(self):
        ...
        if self.video_radio.isChecked():
            ...
            if not os.path.isfile(video_path):
                QMessageBox.warning(self, "无效输入", "所选视频文件无效。")
                return False
            if not video_path.lower().endswith(('.mp4', '.avi', '.mov', '.mkv')):
                QMessageBox.warning(self, "格式错误", "不支持的视频格式。")
                return False
        ...
        if self.camera_radio.isChecked():
            try:
                ...
                QMessageBox.warning(self, "无效输入", "请输入有效的摄像头编号（例如 0）。")
                return False
